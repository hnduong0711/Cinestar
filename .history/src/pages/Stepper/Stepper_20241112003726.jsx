...
<div className="flex flex-col bg-blue-600 p-6 rounded-lg max-w-md mx-auto">
  <h1 className="text-white text-lg font-bold mb-4">Bảng thanh toán</h1>

  <div className="mb-4">
    <div className="text-white font-semibold text-lg">{`Cinestar ${Cinestar.selectedCinema}`}</div>
    <div className="text-gray-200 text-sm">{CinestarAdress.selectedAddress}</div>
  </div>

  <div className="mb-4">
    <div className="text-yellow-300 font-semibold">Bắp nước</div>
    {selectedCombos && selectedCombos.length > 0 ? (
      selectedCombos.map((combo, index) => (
        <div key={index} className="text-white text-sm">{combo.title}</div>
      ))
    ) : (
  ...
  <div className="border-t border-dashed border-gray-200 mt-4 pt-4 flex ">
    <p className="text-yellow-300 font-bold text-lg">SỐ TIỀN CẦN THANH TOÁN</p>
    <p className="text-white font-bold text-xl">{totalPrice ? `${totalPrice} VND` : 'Chưa có giá'}</p>
  </div>
</div>
