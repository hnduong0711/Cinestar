...
const CheckOutFood = ({ selectedCinema, selectedCombos }) => {
    ...
    const totalPrice = selectedCombos.reduce((total, combo, index, array) => {
        const price = parsePrice(combo.price);
        
        // Kiểm tra xem ID đã tồn tại trong tổng chưa
        if (array.findIndex(item => item.id === combo.id) === index) {
            // Nếu ID chưa tồn tại, cộng giá vào tổng
            total += price;
        }
