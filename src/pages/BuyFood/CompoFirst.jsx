        ...
        if (existing) {
          if (quantity <= 0) {
            ...
            return prev.map((combo) =>
              combo.id === food.id ? { ...combo, quantity } : combo
            );
          }
        } else {
          if (quantity > 0) {
        ...
        {Object.keys(groupcomboFoods).map((category) => {
          return (
            <div key={category} className="mb-8">
              <h2 className="text-cinestar-custom-white text-[40px] font-medium pb-[4rem] font-title uppercase">
                {category}
              </h2>
              <div className="flex flex-wrap justify-center">
                {groupcomboFoods[category].map((food) => {
                  return (
                    <div
                      key={food.id}
                      className="flex items-stretch mb-4 w-1/3 group pb-10"
                    >
                      {" "}
                      {/* Thêm class group tại đây */}
                      ...
                      <div>
                        <div className="flex-grow text-left ml-4 flex flex-col">
                          {" "}
                          {/* Căn giữa nội dung */}
                          <h4 className="font-medium text-[16px] font-content text-white text-[18px] transition-colors duration-300 group-hover:text-cinestar-custom-yellow">
                            {food.title.toUpperCase()}
                          </h4>
                          <p className="text-white text-[16px] transition-colors duration-300 font-content">
                            {food.content}
                          </p>
